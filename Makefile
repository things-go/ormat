...
linux:
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build ${opts} ${flags} -o ${execveFile} cmd/ormat/main.go
windows:
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build ${opts} ${flags} -o ${execveFile}.exe cmd/ormat/main.go
mac:
	CGO_ENABLED=1 GOOS=darwin GOARCH=amd64 go build ${opts} ${flags} -o ${execveFile} cmd/ormat/main.go
